...
def post():
    ...
    return render_template('post.html.jinja', form=form, user_search_form=g.user_search_form)

@bp.route('/userpage/<username>')
@login_required    
def user_page(username):
    user = User.query.filter_by(username=username).first()
    return render_template('user_page.html.jinja', title=username, user=user,user_search_form=g.user_search_form)
